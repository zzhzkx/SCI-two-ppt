...
4. **子 Agent 必须真正使用 Agent 工具 spawn**，不能模拟
5. **子 Agent 可以并行执行**，提高效率
6. **每个子 Agent 独立完成任务**，写入对应文件
7. **主 Agent 负责编排和审查**，不直接执行子任务
8. **用户交互贯穿全程**，每步都需要确认
9. **检索式自动生成**：Step 1 时自动生成专业检索式，便于后续文献补充
10. **检索式平衡原则**：精确但不过于精确，使用2-3个核心关键词
